...
/**
 * SideRays — React Bits (JS+CSS variant, dependency: ogl), chuyển sang TS strict.
 * Bổ sung so với bản gốc: respect prefers-reduced-motion (speed = 0 → rays tĩnh).
 * WebGL init/teardown theo IntersectionObserver như bản gốc.
 */
export function SideRays({
  ...
  useEffect(() => {
    ...
    const reduced = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    u.iSpeed.value = reduced ? 0 : speed;
